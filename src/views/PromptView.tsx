...
// Local "Claude is the model" fork: the picker is hidden and the model
// identifier is a fixed sentinel value. Any non-empty string works since
// the bridge ignores it; we just need message creation to keep a `model`
// field so the state machine doesn't break.
const LOCAL_MODEL: Model = 'local-claude';
...
export function PromptView() {
  ...
  // Model is hardcoded in the local fork — the picker is hidden but the rest
  // of the pipeline still expects a `model` field, so we pass a fixed value
  // through `setModel`/state for API compatibility.
  const [model, setModel] = useState<Model>(LOCAL_MODEL);
