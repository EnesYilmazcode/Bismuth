  ...
  async executeOpenscad(
    ...
    try {
      ...
      if (error instanceof Error) {
        throw new Error('Bismuth exited with an error: ' + error.message);
      } else {
        throw new Error('Bismuth exited with an error');
      }
    }
    ...
    if (exitCode === 0) {
      try {
        ...
        if (error instanceof Error) {
          throw new Error('Bismuth cannot read created file: ' + error.message);
        } else {
          throw new Error('Bismuth cannot read created file');
        }
      }
      ...
      throw new OpenSCADError(
        'Bismuth did not exit correctly',
        code,
        this.log.stdErr,
      );
    }
