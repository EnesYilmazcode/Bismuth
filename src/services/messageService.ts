...
export function useCreativeChatMutation({
  ...
  return useMutation({
    ...
    mutationFn: async ({
      ...
      // The bridge has no shared DB with the frontend, so we hand it the
      // conversation history + the user's prompt directly. Identical to
      // what the original edge function loads from Supabase.
      ...
      const promptMessage = (history || []).find(
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/creative-chat`,
        {
          ...
          body: JSON.stringify({
            ...
            newMessageId,
            history: history ?? [],
            prompt: promptMessage?.content ?? null,
          }),
        },
      );
      ...
      // Persist the assistant message into the mock store so any later
      // refetch of ['messages', conversationId] (e.g. triggered by
      // useUpdateMessageOptimisticMutation.onSettled when the user drags a
      // parameter slider) doesn't wipe it out. In the original SaaS this
      // row was created by the edge function; with Claude-in-the-loop the
      // bridge has no DB access, so we upsert here on the client side.
      await supabase
        .from('messages')
        .upsert(finalMessage, { onConflict: 'id' });
...
export function useParametricChatMutation({
  ...
  return useMutation({
    ...
    mutationFn: async ({
      ...
      // Persist into the mock store — see useCreativeChatMutation above for
      // the rationale.
      await supabase
        .from('messages')
        .upsert(finalMessage, { onConflict: 'id' });
